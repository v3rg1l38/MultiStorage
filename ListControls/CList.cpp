...
HWND CList::createList(const HWND & parent,
	const int & xPos, 
	const int & yPos, 
	const int & xSize, 
	const int & ySize,
...
int CList::insertItem(const HWND & list, const int & nRow)
{
	LVITEM lItem;
	int itemIndex;
...
void CList::setFullRowSelect(const HWND & list)
{
	SendMessage(list, LVM_SETEXTENDEDLISTVIEWSTYLE,
		LVS_EX_FULLROWSELECT, LVS_EX_FULLROWSELECT);
}

void CList::clearList(const HWND & list)
{
...
int CList::getSelectedRow(const HWND & list)
{
	int index = SendMessage(list, LVM_GETNEXTITEM, (WPARAM)-1, (LPARAM)LVNI_SELECTED);
	return index;
}

void CList::setSelectedRow(const HWND & list, const int & row)
{
	LVITEM lvItem;
...
void CList::unsetSelectedRow(const HWND & list, const int & row)
{
	LVITEM lvItem;
	lvItem.stateMask = LVIS_SELECTED | LVIS_FOCUSED | LVNI_SELECTED;
	lvItem.state = 0;
...
int CList::getItemCount(const HWND & list)
{
	int count = SendMessage(list, LVM_GETITEMCOUNT, 0, 0);
	return count;
}

void CList::setBkColor(const HWND & list, COLORREF col)
{
	SendMessage(list, LVM_SETBKCOLOR, 0, col);
}

void CList::setTxtBkColor(const HWND  & list, COLORREF col)
{
